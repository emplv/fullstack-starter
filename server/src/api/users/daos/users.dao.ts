...
class UsersDao {
    ...
    getUserByUsername(username: string): Promise<Omit<UserDto, 'password'>> {
        return this.User.findOne({ username }).exec();
    }

    getUserByUsernameWithPassword(username: string): Promise<UserDto> {
        return this.User.findOne({ username }, '_id username name permissionFlags +password').exec();
    }
